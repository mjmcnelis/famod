...
void find_anisotropic_variables(double e, double pl, double pt, double * mass, int * degeneracy, int * sign, int number_resonances, double * pbar_rootF, double * pbar_weightF, double * pbar_rootJ, double * pbar_weightJ, int pbar_pts, double *lambda, double *ax, double *az)
{
	const double Ea = e;
	const double PTa = pt;
	const double PLa = pl;
	...
	// in case I need to set root/weight arrays again
	// // a = 2 Gauss Laguerre roots/weights (for F)
	// double pbar_rootF[gla_pts];
	// double pbar_weightF[gla_pts];
	// // a = 3 gauss laguerre roots/weights (for J)
	// double pbar_rootJ[gla_pts];
	// double pbar_weightJ[gla_pts];
	...
	double axi = *ax;
	double azi = *az;
	double alphaBai = *alphaBa;

	const int n = 4; 										 // dimension space
	double X[n] = {lambdai, axi, azi, alphaBai};			 // initial guess vector
	...
	// prefactors and factors of F/J elements: to be evaluated at ith iteration of X
 	double commonfactori;
  	...
  	double factorEai, factorPTai, factorPLai, factornBai;
	...
	do
	{
	    ...
	    for(int k = 0; k < number_resonances; k++)
	    {
	    	...
	    	if(baryon[k] != 0)
	    		nBai += dof * factornBai * Gauss_Aniso_1D(nBa_integrand, pbar_rootN, pbar_weightN, pbar_pts, axi, azi, mass[k]/lambdai, aBai, baryon[k], sign[k]);
	    	Eai += dof * factorEai * Gauss_Aniso_1D(Ea_integrand, pbar_rootF, pbar_weightF, pbar_pts, axi, azi, mass[k]/lambdai, aBai, baryon[k], sign[k]);
	    	PTai += dof * factorPTai * Gauss_Aniso_1D(PTa_integrand, pbar_rootF, pbar_weightF, pbar_pts, axi, azi, mass[k]/lambdai, aBai, baryon[k], sign[k]);
	    	PLai += dof * factorPLai * Gauss_Aniso_1D(PLa_integrand, pbar_rootF, pbar_weightF, pbar_pts, axi, azi, mass[k]/lambdai, aBai, baryon[k], sign[k]);
	    }
    	...
    	// sum over resonances (don't forget degeneracy)

	    for(int k = 0; k < number_resonances; k++)
	    {
	    	dof = (double)degeneracy[k];

	    	I2001 += dof * factorI2001 * Gauss_Aniso_1D(I2001_integrand, pbar_rootJ, pbar_weightJ, pbar_pts, axi, azi, mass[k]/lambdai, sign[k]);
		    I2011 += dof * factorI2011 * Gauss_Aniso_1D(I2011_integrand, pbar_rootJ, pbar_weightJ, pbar_pts, axi, azi, mass[k]/lambdai, sign[k]);
		    I2201 += dof * factorI2201 * Gauss_Aniso_1D(I2201_integrand, pbar_rootJ, pbar_weightJ, pbar_pts, axi, azi, mass[k]/lambdai, sign[k]);
		    //I401m1 += dof * factorI401m1 * Gauss_Aniso_1D(I401m1_integrand, pbar_rootJ, pbar_weightJ, pbar_pts, axi, azi, mass[k]/lambdai, sign[k]);
		    //I420m1 += dof * factorI420m1 * Gauss_Aniso_1D(I420m1_integrand, pbar_rootJ, pbar_weightJ, pbar_pts, axi, azi, mass[k]/lambdai, sign[k]);
		    I402m1 += dof * factorI402m1 * Gauss_Aniso_1D(I402m1_integrand, pbar_rootJ, pbar_weightJ, pbar_pts, axi, azi, mass[k]/lambdai, sign[k]);
		    I421m1 += dof * factorI421m1 * Gauss_Aniso_1D(I421m1_integrand, pbar_rootJ, pbar_weightJ, pbar_pts, axi, azi, mass[k]/lambdai, sign[k]);
		    I440m1 += dof * factorI440m1 * Gauss_Aniso_1D(I440m1_integrand, pbar_rootJ, pbar_weightJ, pbar_pts, axi, azi, mass[k]/lambdai, sign[k]);
	    }
	    ...
	    J[0][3] = bn_J2000;   // need to evaluate this guy

	    // row 2
	    J[1][0] = I2011/lambdai2;
	    J[1][1] = 4.0*I402m1/lambdaiaxi3;
	    J[1][2] = I421m1/lambdaiazi3;       // identity J421m1 = ax2*az2*lambda*(PTa-PLa)/(ax2-az2) isn't numerically stable
	    //J[1][2] = (PTai - PLai)*axi2/(axi2-azi2)/azi;


	    J[1][3] = bn_J2010;   // need to evaluate this guy
	    ...
	    J[2][3] = bn_J2200;   // need to evaluate this guy
	    ...
	    J[3][1] = 2.0*nBai/axi;
	    J[3][2] = nBai/azi;
	    J[3][3] = bn2_J1000;  // need to evaluate this guy
